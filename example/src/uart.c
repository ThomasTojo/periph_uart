...
static void vUART_ReadTask1(void *pvParameters) {
	...
	while(1)
	{
		while(count<10)
		{
			...
			//if the the uart receive buffer is full
			if(Chip_UART_ReadLineStatus(LPC_UART)&UART_LSR_RDR)
			{
				buffer[count] = Chip_UART_ReadByte(LPC_UART);
				count++;
...
static void vUART_WriteTask2(void *pvParameters) {
	...
	while(1)
	{
		if(count>0)
		{
			while(*str)
			{
				if(Chip_UART_ReadLineStatus(LPC_UART)&UART_LSR_TEMT)
				{
					Chip_UART_Send(LPC_UART,str,1);
					*str++;
				}
			}
			str = "The Data is:";
			...
			while(count)
			{
				if(Chip_UART_ReadLineStatus(LPC_UART)&UART_LSR_TEMT)
				{
					Chip_UART_Send(LPC_UART,&buffer[9-count],1);
					count--;
				}
			}
/*			for(i=0;i<10;i++)
